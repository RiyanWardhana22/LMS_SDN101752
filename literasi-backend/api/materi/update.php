...
try {
            ...
            if (!empty($id) && !empty($judul)) {
                        $db->beginTransaction();
                        $query = "UPDATE materi SET judul = :judul, konten = :konten, mata_pelajaran = :mapel, kelas = :kelas, visibilitas = :visibilitas WHERE id = :id";
                        ...
                        $stmt_del->execute();
                        if (!empty($media)) {
                                    $query_ins_media = "INSERT INTO materi_media (materi_id, tipe_media, url_atau_path) VALUES (:materi_id, :tipe, :url)";
                                    $stmt_ins = $db->prepare($query_ins_media);
                                    foreach ($media as $m) {
                                                $tipe = isset($m->type) ? $m->type : 'unknown';
                                                $url = isset($m->url) ? $m->url : '';
                                                $stmt_ins->bindValue(':materi_id', $id);
                                                $stmt_ins->bindValue(':tipe', $tipe);
                                                $stmt_ins->bindValue(':url', $url);
                                                $stmt_ins->execute();
                                    }
                        }
