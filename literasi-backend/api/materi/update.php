...
try {
    ...
    if (!empty($id) && !empty($judul) && !empty($rombel_id)) {
        ...
        // Update materi
        $query = "UPDATE materi 
                  SET judul = :judul, konten = :konten, rombel_id = :rombel_id, 
                      mata_pelajaran = :mapel, kelas = :kelas, visibilitas = :visibilitas 
                  WHERE id = :id";
        ...
        // Insert media baru (termasuk is_ar_output, nama_file, dan model_config)
        if (!empty($media)) {
            $query_ins_media = "INSERT INTO materi_media 
            ...
            foreach ($media as $m) {
                ...
                $model_config = null;
                if (
                    ($tipe === 'model_3d' || $tipe === 'model_3d_animated') 
                    && isset($m->model_config)
                ) {
                    $model_config = is_string($m->model_config) 
                        ? $m->model_config 
                        : json_encode($m->model_config);
                }
                
                $stmt_ins->bindValue(':materi_id', $id);
                $stmt_ins->bindValue(':tipe_media', $tipe);
                $stmt_ins->bindValue(':url_atau_path', $url);
                $stmt_ins->bindValue(':is_ar_output', $is_ar_output);
