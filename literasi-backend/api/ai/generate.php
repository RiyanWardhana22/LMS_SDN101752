...
register_shutdown_function(function () {
    ...
    if ($error !== NULL && in_array($error['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR])) {
        echo json_encode([
            "status" => "error",
            "message" => "Server PHP Crash: " . $error['message']
        ]);
        exit;
    }
});
...
if (!ini_get('allow_url_fopen')) {
    echo json_encode(["status" => "error", "message" => "Fitur 'allow_url_fopen' di PHP Anda nonaktif. Wajib menyala karena cURL tidak digunakan."]);
    exit;
}
...
foreach ($api_keys as $index => $key) {
    $url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent?key=" . trim($key);
    ...
    if ($response !== false && $http_code == 200) {
        ...
        $error_msg = "Gagal di API ke-" . ($index + 1) . " (HTTP $http_code). Respons: " . $response;
        continue; 
    }
}
...
if ($success) {
    ...
    echo json_encode(["status" => "error", "message" => "Semua API sedang sibuk atau limit (Error 429).", "debug" => $error_msg]);
}
?>