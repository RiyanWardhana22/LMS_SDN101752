    ...
    const fetchDashboardData = async () => {
      try {
        const response = await fetch(
          apiEndpoint("api/admin/dashboard_stats.php"),
        );
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-white rounded-2xl shadow-[0_2px_12px_rgba(26,26,46,0.06)] p-6 border border-neutral-100 hover:-translate-y-1 transition-transform">
              ...
              <p className="text-4xl font-black text-neutral-900">
                {stats?.siswa_aktif_mingguan || 0}
              </p>
              <p className="text-xs text-[#2ecc71] font-bold mt-2">
                Siswa mengakses & kerjakan tugas
              </p>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-[0_2px_12px_rgba(26,26,46,0.06)] p-6 border border-neutral-100 hover:-translate-y-1 transition-transform">
              ...
              <p className="text-4xl font-black text-neutral-900">
                {stats?.guru_aktif?.reduce(
                  (acc, curr) => acc + parseInt(curr.total_materi),
                  0,
                ) || 0}
              </p>
              <p className="text-xs text-[#3498db] font-bold mt-2">
            ...
            <div className="bg-white rounded-2xl shadow-[0_2px_12px_rgba(26,26,46,0.06)] p-6 border border-neutral-100 hover:-translate-y-1 transition-transform">
              ...
              <p className="text-xs text-neutral-500 font-bold mt-2">
                Kontributor materi terbanyak
              </p>
            </div>

            {/* Card 4: Status Darurat (SUDAH DINAMIS) */}
            <div
              className={`rounded-2xl shadow-[0_2px_12px_rgba(26,26,46,0.06)] p-6 border-2 transition-all duration-300 ${isEmergency ? "bg-red-50 border-red-500" : "bg-white border-[#e74c3c]/20 hover:border-[#e74c3c]/50"}`}
            >
              <div className="flex items-center gap-3 text-neutral-500 mb-4">
                <div
                  className={`p-2 rounded-lg ${isEmergency ? "bg-red-500 text-white animate-pulse" : "bg-[#fdedec] text-[#e74c3c]"}`}
                >
                  <WarningCircle weight="fill" size={24} />
                </div>
                <h3
                  className={`font-bold text-sm ${isEmergency ? "text-red-700" : "text-[#e74c3c]"}`}
                >
                  Status Mode Darurat
                </h3>
              </div>
              <p
                className={`text-2xl font-black mt-2 ${isEmergency ? "text-red-700" : "text-[#e74c3c]"}`}
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-[0_2px_12px_rgba(26,26,46,0.06)] border border-neutral-100">
              <h3 className="text-lg font-bold text-neutral-800 mb-6">
                Tren Aktivitas Belajar (Bulanan)
              </h3>
              <div className="h-72 w-full">
