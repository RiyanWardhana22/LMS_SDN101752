...
export default function UserManagement() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const url = filterRole
        ? apiEndpoint(`api/admin/users.php?role=${filterRole}`)
        : apiEndpoint("api/admin/users.php");
  ...
  const handleSubmitForm = async (e) => {
    ...
    try {
      const response = await fetch(apiEndpoint("api/admin/users.php"), {
        method: method,
  ...
  const handleDeleteUser = async (id, nama) => {
    ...
    try {
      const response = await fetch(
        apiEndpoint(`api/admin/users.php?id=${id}`),
        {
          method: "DELETE",
        },
      );
  ...
  const handleDownloadTemplate = () => {
    const csvContent =
      "data:text/csv;charset=utf-8,Nama,PIN,ID_Kelas\nBudi Santoso,1234,1\nSiti Aminah,5678,1";
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        ...
        if (formattedData.length === 0) {
          alert(
            "Format CSV salah atau kosong. Pastikan memiliki kolom: Nama, PIN, dan ID_Kelas.",
          );
          setIsSubmitting(false);
          e.target.value = null;
          return;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-neutral-500 font-medium">
            Kelola data siswa, guru, dan admin sekolah.
          </p>
        </div>
        <div className="flex flex-wrap gap-3">
          <button
            onClick={handleDownloadTemplate}
            className="flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 font-bold rounded-xl hover:bg-gray-200 transition-colors shadow-sm"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-[#2ecc71] text-white font-bold rounded-xl hover:bg-[#27ae60] transition-colors shadow-sm disabled:opacity-50"
          >
            <FileCsv weight="bold" size={20} />{" "}
            {isSubmitting ? "Memproses..." : "Import CSV"}
          </button>
          <button
            onClick={handleAddClick}
            className="flex items-center gap-2 px-4 py-2 bg-[#3498db] text-white font-bold rounded-xl hover:bg-[#2980b9] transition-colors shadow-sm"
          >
      ...
      <div className="bg-white rounded-2xl shadow-[0_2px_12px_rgba(26,26,46,0.06)] border border-neutral-100 overflow-hidden">
        <div className="p-6 border-b border-neutral-100 flex gap-4">
          <select
            ...
            className="bg-gray-50 border border-gray-200 text-gray-700 text-sm rounded-lg focus:ring-blue-500 block w-full md:w-48 p-2.5 font-medium cursor-pointer"
          >
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td
                    colSpan="4"
                    className="px-6 py-8 text-center text-gray-400 font-medium animate-pulse"
                  >
                    Memuat data...
                  </td>
                </tr>
                ...
                users.map((user) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold ${user.role === "admin" ? "bg-purple-100 text-purple-700" : user.role === "guru" ? "bg-orange-100 text-orange-700" : "bg-blue-100 text-blue-700"}`}
                      >
                        ...
                        <div>
                          <span className="block text-xs font-bold text-gray-500">
                            ...
                            <span className="text-gray-800">
                              {user.nama_kelas || `ID ${user.rombel_id || "-"}`}
                            </span>
                          </span>
                          <span className="block text-xs font-bold text-gray-500">
                            PIN:{" "}
                            <span className="text-red-500">{user.pin}</span>
                          </span>
                        </div>
                      ) : (
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex justify-between items-center p-6 border-b border-gray-100">
              <h3 className="text-xl font-bold text-gray-900">
                ...
                {formData.role === "siswa" ? "Siswa" : "Staf"}
              </h3>
              <button
                onClick={closeModal}
            ...
            <form onSubmit={handleSubmitForm} className="p-6 space-y-4">
              <div>
                ...
                <select
                  ...
                  disabled={isEditMode}
                  className="w-full p-2.5 bg-gray-50 border rounded-lg focus:ring-2 focus:ring-blue-500 font-bold text-blue-700 disabled:opacity-50"
                >
                  <option value="siswa">🧑‍🎓 Siswa</option>
                  <option value="guru">👨‍🏫 Guru</option>
                  <option value="admin">👨‍💻 Admin</option>
              ...
              <div>
                ...
                <input
                  ...
                  value={formData.nama}
                  onChange={handleInputChange}
                  className="w-full p-2.5 bg-gray-50 border rounded-lg focus:ring-2 focus:ring-blue-500"
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={formData.pin}
                      onChange={handleInputChange}
                      className="w-full p-2.5 bg-gray-50 border rounded-lg tracking-[0.5em] font-bold text-center"
                      placeholder="1234"
                    />
                  </div>
                </div>
              ...
              <div className="pt-4 flex gap-3">
                <button
                  type="button"
                  onClick={closeModal}
                  className="flex-1 px-4 py-2.5 bg-gray-100 font-bold rounded-xl hover:bg-gray-200"
                >
                  Batal
                </button>
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="flex-1 px-4 py-2.5 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700"
                >
