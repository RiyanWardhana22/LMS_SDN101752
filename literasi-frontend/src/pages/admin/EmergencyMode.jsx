...
import { apiEndpoint } from "../../config/api"; // Siap digunakan jika API tersedia
...
export default function EmergencyMode() {
  ...
  useEffect(() => {
    // Nanti ini dihubungkan ke API, misal: fetch(apiEndpoint('api/admin/settings.php'))
  ...
  const toggleEmergency = () => {
    ...
    setTimeout(() => {
      ...
      if (newStatus) {
        alert(
          "🚨 MODE DARURAT DIAKTIFKAN! Seluruh sistem kini beralih ke mode Belajar Dari Rumah.",
        );
      } else {
        alert(
          "✅ Mode Darurat dimatikan. Sistem kembali ke mode tatap muka normal.",
        );
      }
    }, 800);
  };
  ...
  const handleSchedule = (e) => {
    ...
    alert(
      `Berhasil! Mode darurat dijadwalkan otomatis aktif pada: ${scheduleDate}`,
    );
    setScheduleDate("");
  };
    ...
    <DashboardLayout role="admin" title="Mode Darurat">
      <div className="mb-8">
        <h2 className="text-3xl font-black text-neutral-900 mb-2">
          Manajemen Mode Darurat
        </h2>
        <p className="text-neutral-500 font-medium">
          Pusat kendali untuk mengalihkan sistem ke mode Belajar Dari Rumah
          secara instan.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div
          className={`p-8 rounded-3xl border-2 transition-all duration-500 ${isEmergency ? "bg-red-50 border-red-500 shadow-[0_0_40px_rgba(239,68,68,0.2)]" : "bg-white border-gray-200 shadow-sm"}`}
        >
          <div className="flex flex-col items-center text-center">
            <div
              className={`p-6 rounded-full mb-6 transition-colors duration-500 ${isEmergency ? "bg-red-500 text-white animate-pulse" : "bg-gray-100 text-gray-400"}`}
            >
              {isEmergency ? (
            ...
            <h3
              className={`text-2xl font-black mb-2 ${isEmergency ? "text-red-600" : "text-gray-800"}`}
            >
        ...
        <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
          ...
          <p className="text-gray-500 font-medium mb-6">
            Jadwalkan aktivasi Mode Darurat jika ada prediksi cuaca buruk
            (banjir, badai) atau bencana alam di hari-hari mendatang.
          </p>
          ...
          <div className="mt-8 p-4 bg-yellow-50 border border-yellow-200 rounded-xl flex gap-3">
            ...
            <p className="text-sm font-medium text-yellow-800">
              Fitur ini akan menonaktifkan fitur tatap muka di perangkat siswa
              dan menampilkan pesan peringatan di halaman login mereka.
            </p>
          </div>
        </div>
      </div>
